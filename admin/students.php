...
.badge-linked { background:#e6f9ec; color:#0f5132; }
.badge-not-linked { background:#fff3cd; color:#664d03; }
...
<form method="get" class="filter-bar">
  ...
  <a href="<?= BASE_URL ?>admin/bulk-id-cards.php">🪪 Print ID Cards</a>
</form>
...
<input type="checkbox"
  ...
  <td>
    <?php if ($row['parent_id']): ?>
      <span class="badge badge-linked">Linked</span><br>
      <!--<a class="link-btn"-->
      <!--   href="<?= BASE_URL ?>admin/parent-details.php?id=<?= $row['parent_id'] ?>">-->
      <!--   👁 View Parent-->
      <!--</a>-->
    <?php else: ?>
      <span class="badge badge-not-linked">Not Linked</span><br>
      <a class="link-btn"
         href="<?= BASE_URL ?>admin/link-parent.php?student_id=<?= $student_id ?>">
         🔗 Link Parent
      </a>
    <?php endif; ?>
  </td>
  ...
  <td>
    ...
    <a href="<?= BASE_URL ?>admin/student-card.php?id=<?= $student_id ?>" target="_blank">🎴 ID Card</a>
...
<?php for($p = 1; $p <= $pages; $p++): ?>
  <a href="?page=<?= $p ?>&q=<?= urlencode($search) ?>&status=<?= urlencode($statusFilter) ?>"
     class="<?= $p==$page?'active':'' ?>">
     <?= $p ?>
  </a>
<?php endfor; ?>
...
<?php endif; ?>
<script>
document.querySelectorAll('.schoolToggle').forEach(box => {

box.addEventListener('change', function(){

let student_id = this.dataset.id;
let value = this.checked ? 1 : 0;

fetch('<?= BASE_URL ?>backend/update-school.php',{
method:'POST',
