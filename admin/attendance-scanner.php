...
form.addEventListener('submit', e => {
  ...
  fetch(form.action, { method:'POST', body: formData })
    ...
    .then(data => {
      ...
      /* ===============================
         1️⃣ FEE PENDING POPUP
      =============================== */
      if (data.fee_pending) {

        if (confirm(`${data.name} has pending or partial fees.\n\nAllow attendance?`)) {

          const forceData = new FormData(form);
          forceData.append('force', '1');

          fetch(form.action, { method:'POST', body: forceData })
            .then(res => res.json())
            .then(resp => {
              if (resp.success) {
                showSuccess(resp);
          ...
          showError("❌ Attendance rejected due to pending fees");
        }
...
function showError(msg) {
  resultBox.innerHTML = `<p style="color:#b91c1c">⚠️ ${msg}</p>`;
}
...
function loadTodayAttendance() {
  fetch("<?= BASE_URL ?>backend/fetch_today_attendance.php")
    ...
    .then(rows => {
      table.innerHTML = '';
      if (!rows.length) {
        table.innerHTML = `<tr><td colspan="6" style="text-align:center;color:gray;">No attendance marked yet today.</td></tr>`;
        return;
      }
      rows.forEach(r => {
        table.innerHTML += `
