...
$stmt = $conn->prepare("
  ...
  SET file_path=?, note=?, submitted_at=?
  WHERE id=?
");

$stmt->bind_param("sssi",$filePath,$note,$currentDateTime,$submission['id']);
  } else {
$currentDateTime = date('Y-m-d H:i:s');
...
<form method="post" enctype="multipart/form-data" id="submitHomeworkForm">
<input 
    type="file" 
    name="answer"
...
<button id="submitHomeworkBtn">
    <?= $submission?'🔄 Update':'📤 Submit Homework' ?>
</button>
</form>
...
const submitHomeworkForm =
    document.getElementById('submitHomeworkForm');

const uploadLoader =
    document.getElementById('uploadLoader');

const submitHomeworkBtn =
    document.getElementById('submitHomeworkBtn');

submitHomeworkForm.addEventListener('submit', function () {

    // Show loader
    uploadLoader.style.display = 'flex';

    // Disable button
    submitHomeworkBtn.classList.add('loading');

    submitHomeworkBtn.innerHTML = '⏳ Uploading...';
