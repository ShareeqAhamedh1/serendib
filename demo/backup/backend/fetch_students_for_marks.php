...
LEFT JOIN exam_marks em 
  ON em.student_id=s.id AND em.exam_id=$exam_id AND em.subject_id=$subject_id
    ...
    <tbody>
      <?php while($r=$res->fetch_assoc()): ?>
        <tr>
          ...
          <td><input type="number" name="marks[<?= $r['id'] ?>]" value="<?= esc($r['marks_obtained'] ?? '') ?>" min="0" max="100" step="0.5" required></td>
          <td><?= esc($r['grade'] ?? '-') ?></td>
          <td><?= esc($r['status'] ?? '-') ?></td>
        </tr>
      <?php endwhile; ?>
